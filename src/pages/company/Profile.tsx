...
export const CompanyProfileView: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(false);
    if (!res.success) {
      setSaveError(res.error || 'Failed to update company profile');
      return;
    }
    setSaved(true);
    ...
    <div className="space-y-8 antialiased">
      <div className="border-b border-surface-border pb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight flex items-center gap-2.5">
            ...
            Company Enterprise Credentials
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Manage your company profile, recruitment contact, and verification status.
          </p>
        </div>
      ...
      {saved && (
        <div className="p-3 rounded-lg bg-emerald-950/60 border border-emerald-800 text-emerald-300 text-xs font-semibold flex items-center gap-2">
          <Check className="w-4 h-4" /> Company profile updated!
        </div>
      )}

      <form onSubmit={handleSave} className="space-y-6">
        <Card className="space-y-6">
          <h2 className="text-base font-bold text-white border-b border-surface-border pb-3">Corporate Information</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Verified Business Email</label>
        ...
        <Card className="space-y-4">
          <h2 className="text-base font-bold text-white border-b border-surface-border pb-3">
            Incorporation & Verification Document
          </h2>
          <div className="p-4 bg-background rounded-lg border border-surface-border flex justify-between items-center text-xs">
            <div>
              <p className="font-bold text-white">apex_systems_incorporation_certificate.pdf</p>
              <p className="text-slate-400 mt-0.5">Status: {isVerified ? 'Approved' : 'Under Verification Audit'}</p>
            </div>
